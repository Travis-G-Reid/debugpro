...
def _print_code_context(filename: str, lineno: int, error_color: str, reset_color: str):
    ...
    for i in range(start_line, end_line + 1):
        ...
        if context_line:  # Only print if the line exists
            ...
            if i == lineno:
                print(f"{prefix}{i}: {error_color}{context_line.rstrip()}{reset_color}")
            else:
...
def _custom_excepthook(exc_type: Type[BaseException], 
    ...
    if tb:
        ...
        if issubclass(exc_type, KeyError):
            ...
            for name, val in frame.f_locals.items():
                if isinstance(val, dict) and line and name in line:
                    try:
                        ...
                        # Prepare advice
                        exception_advice = [
                            f"Check if the key exists before accessing it using: if {repr(key_str)} in {name}",
                            f"Use .get() method to provide a default value: {name}.get({repr(key_str)}, default_value)",
                            f"Consider adding the key with a default value: {name}[{repr(key_str)}] = default_value"
                        ]
                        break
                    except Exception as e:
            ...
            # Special handling for IndexError and TypeError
            var_name = None
            for name, val in frame.f_locals.items():
                ...
                for name, val in frame.f_locals.items():
                    if name in line:
                        try:
                            ...
                            # Prepare advice
                            exception_advice = [
                                f"Check if object is of expected type: isinstance({name}, expected_type)",
                                f"Use hasattr() to check if attribute exists: hasattr({name}, '{attr_name}')",
                                f"Use getattr() with default: getattr({name}, '{attr_name}', default_value)"
                            ]
                            break
                        except Exception as e:
        ...
        if exception_details:
            ...
            if "dictionary" in exception_details:
                ...
                # print(f"{YELLOW}Object Type That's Missing Attribute:{RESET} '{exception_details['missing_attribute']}'")
